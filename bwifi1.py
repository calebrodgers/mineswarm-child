...
def format_position_data(x, y):
    ...
    return "Position: x={:.2f}, y={:.2f}".format(x, y)
...
def listen_udp():
    ...
    return None
def generate_rectangle_wave_waypoints(length, width, flag, num_waves):
    """
    Generate waypoints for a series of connected rectangles forming a wave-like pattern.

    Parameters:
    length (float): The length of each rectangle wave (in meters).
    width (float): The width of each rectangle wave (in meters).
    flag (int): Direction flag, 0 for left and 1 for right.
    num_waves (int): Number of rectangle waves to generate.

    Returns:
    list: List of waypoints in the format [distance, angle], where distance is in meters and angle in degrees.
    """
    waypoints = []

    # First move to the initial position of the wave pattern
    waypoints.append([0.2,0])
    waypoints.append([length, 0])

    # Generate the waves
    for _ in range(num_waves):
        if flag == 0:  # Left turns
            waypoints.append([0, 90])  # Turn left
            waypoints.append([width, 0])  # Move along the width
            waypoints.append([0, 90])  # Turn left
        else:  # Right turns
            waypoints.append([0, -90])  # Turn right
            waypoints.append([width, 0])  # Move along the width
            waypoints.append([0, -90])  # Turn right
        waypoints.append([length, 0])  # Move back along the length

        # Reverse the direction for the next wave
        if flag == 0:
            waypoints.append([0, -90])  # Turn right
            waypoints.append([width, 0])  # Move along the width
            waypoints.append([0, -90])  # Turn right
    ...
    waypoints.append([0, 180])
    waypoints.append([length,0])
    waypoints.append([0, 90])# Turn around to face the starting direction
    waypoints.append([width* num_waves * 2, 0])  # Move back the total length covered
    waypoints.append([0, -90])
    waypoints.append([0.2,0])
...
def plan_path(width, height, robot_width):
    ...
    # Create a waypoint for getting off of the platform
    waypoints = [[0.15, 0]] + waypoints
    ...
    while True:
        ...
        if command:
            if command == "1":
                ...
                # Execute Task 1
                motors.set_speeds(robot.Motors.MAX_SPEED/6, robot.Motors.MAX_SPEED/6)
                time.sleep(3)
                motors.off()
                motors.set_speeds(-robot.Motors.MAX_SPEED/6, -robot.Motors.MAX_SPEED/6)
                time.sleep(3)
            ...
            # Add more elif blocks for additional commands if needed
                ...
                waypoints = generate_rectangle_wave_waypoints(area_height, area_width, flag, num_waves)
